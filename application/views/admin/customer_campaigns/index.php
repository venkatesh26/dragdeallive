					...
					<div class="widget-content">
						...
						<div id="module-search" class="pull-right col-md-4">
							<div class="quick-search-container">
								...
								<?php 
								 echo form_open(ADMIN.'/customer_campaigns/'.$type.'/1');
								...
								echo ' '.anchor(ADMIN.'/customer_campaigns/'.$type.'/1/reset', 'Clear' ,array('title'=>'Reset', 'class'=>'btn btn-sm btn-default'));
								?>
								</span>
								<?php echo form_close(); ?>
								</div>
						...
						<!-- /widget -->
					<?php echo form_open(base_url().ADMIN.'/customer_campaigns/bulkautions/'.$pagestatus.'/'.$pagingstatus.'?sortingfied='.$fieldssort.'&sortype='.$ordersort); ?>
						...
						<div class="widget-content">
						  <table class="table table-striped table-bordered" id="rowclick1">
							<thead>
							  ...
							  <th><?php $sort_url = base_url().ADMIN.'/customer_campaigns/'.$type.'/'.$page_num.'/created/'.$order;   if($sort_seg == 'created') {  echo anchor($sort_url ,'Created'.$sort_img, $sort_att );  } else { echo anchor($sort_url,'Created'.$sort_def_img, $sort_att ); }?> </th>
							   <th class="td-actions new-items2"><?php $sort_url = base_url().ADMIN.'/customer_campaigns/'.$type.'/'.$page_num.'/title/'.$order;   if($sort_seg == 'created') {  echo anchor($sort_url ,'Title'.$sort_img, $sort_att );  } else { echo anchor($sort_url,'Title'.$sort_def_img, $sort_att ); }?> </th>
							...
							?>
							  <tr class="check-select">
								...
								<td class="td-actions">
								<?php 
								echo anchor(base_url().ADMIN.'/customer_campaigns/view/'.$vals['id'].'?pagemode='.$pagestatus.'&modestatus='.$pagingstatus.'&sortingfied='.$fieldssort.'&sortype='.$ordersort,'<i class="btn-icon-only icon-list-alt"> </i>',array('class'=>'btn btn-big','title'=>'View'));
								echo anchor(base_url().ADMIN.'/customer_campaigns/delete/'.$vals['id'].'?pagemode='.$pagestatus.'&modestatus='.$pagingstatus.'&sortingfied='.$fieldssort.'&sortype='.$ordersort,'<i class="btn-icon-only icon-trash"> </i>',array('class'=>'btn btn-big delete-con','title'=>'Delete'));
								?>
								</td>
							  </tr>
							<?php }
