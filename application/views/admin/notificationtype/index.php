	      	...
	      	<div class="span12">      		
	      		<div class="widget ">
	      			<div class="widget-header">
						...
						?>
						<span class="create_new"><?php echo anchor(base_url().ADMIN.'/notification_type/add?pagemode='.$pagestatus.'&modestatus='.$pagingstatus.'&sortingfied='.$fieldssort.'&sortype='.$ordersort,'Create',array('class'=>'btn btn-suc','title'=>'Create')); ?></span>
	  				</div> <!-- /widget-header -->
					<div class="widget-content">
						...
						<div id="module-search" class="pull-right col-md-4">
							<div class="quick-search-container">
								...
								<?php 
								 echo form_open(ADMIN.'/notificationtype/'.$type.'/1');
								...
								echo ' '.anchor(ADMIN.'/notificationtype/'.$type.'/1/reset', 'Clear' ,array('title'=>'Clear', 'class'=>'btn btn-sm btn-default'));
								?>
								</span>
								<?php echo form_close(); ?>
								</div>
						...
						<!-- /widget -->
					<?php echo form_open(base_url().ADMIN.'/notificationtype/bulkautions/'.$pagestatus.'/'.$pagingstatus.'?sortingfied='.$fieldssort.'&sortype='.$ordersort); ?>
						...
						<div class="widget-content">
						  <table class="table table-striped table-bordered" id="rowclick1">
							<thead>
							  <tr>
							   ...
							   if($this->uri->segment(3) !="new") { ?>
								...
								<th><?php $sort_url = base_url().ADMIN.'/notificationtype/'.$type.'/'.$page_num.'/name/'.$order;   if($sort_seg == 'name') {  echo anchor($sort_url ,'Name'.$sort_img, $sort_att );  } else { echo anchor($sort_url,'Name'.$sort_def_img, $sort_att ); }?></th>
								<th class="create-wt"><?php $sort_url = base_url().ADMIN.'/notificationtype/'.$type.'/'.$page_num.'/created/'.$order;   if($sort_seg == 'created') {  echo anchor($sort_url ,'Created'.$sort_img, $sort_att );  } else { echo anchor($sort_url,'Created'.$sort_def_img, $sort_att ); }?> </th>
							...
							if(count($notificationtype)) {
								foreach($notificationtype as $vals) { $i++; 
							?>
							  <tr class="check-select">
								...
								<td class="td-actions">
								<?php 
								echo anchor(base_url().ADMIN.'/notification_type/edit/'.$vals['id'].'?pagemode='.$pagestatus.'&modestatus='.$pagingstatus.'&sortingfied='.$fieldssort.'&sortype='.$ordersort,'<i class="btn-icon-only icon-edit"> </i>',array('class'=>'btn btn-big','title'=>'Edit'));
								echo anchor(base_url().ADMIN.'/notificationtype/delete/'.$vals['id'].'?pagemode='.$pagestatus.'&modestatus='.$pagingstatus.'&sortingfied='.$fieldssort.'&sortype='.$ordersort,'<i class="btn-icon-only icon-trash"> </i>',array('class'=>'btn btn-big delete-con','title'=>'Delete'));
								?>
								</td>
							  </tr>
							<?php }
