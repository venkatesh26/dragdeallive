					...
					<div class="widget-content">
						...
						<div id="module-search" class="pull-right col-md-4">
							<div class="quick-search-container">
								...
								<?php 
								 echo form_open(ADMIN.'/customer_remainder_histroy/'.$type.'/1');
								...
								echo ' '.anchor(ADMIN.'/customer_remainder_histroy/'.$type.'/1/reset', 'Clear' ,array('title'=>'Reset', 'class'=>'btn btn-sm btn-default'));
								?>
								</span>
								<?php echo form_close(); ?>
								</div>
						...
						<!-- /widget -->
					<?php echo form_open(base_url().ADMIN.'/customer_remainder_histroy/bulkautions/'.$pagestatus.'/'.$pagingstatus.'?sortingfied='.$fieldssort.'&sortype='.$ordersort); ?>
						  ...
						  <table class="table table-striped table-bordered" id="rowclick1">
							...
							?>
							  <tr class="check-select">
								...
								<td class="td-actions">
								<?php 
								echo anchor(base_url().ADMIN.'/customer_remainder_histroy/view/'.$vals['id'].'?pagemode='.$pagestatus.'&modestatus='.$pagingstatus.'&sortingfied='.$fieldssort.'&sortype='.$ordersort,'<i class="btn-icon-only icon-list-alt"> </i>',array('class'=>'btn btn-big','title'=>'View'));
								echo anchor(base_url().ADMIN.'/customer_remainder_histroy/delete/'.$vals['id'].'?pagemode='.$pagestatus.'&modestatus='.$pagingstatus.'&sortingfied='.$fieldssort.'&sortype='.$ordersort,'<i class="btn-icon-only icon-trash"> </i>',array('class'=>'btn btn-big delete-con','title'=>'Delete'));
								?>
								</td>
							  </tr>
							<?php }
