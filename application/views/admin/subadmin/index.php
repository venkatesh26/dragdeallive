	      	...
	      	<div class="span12">      		
	      		<div class="widget ">
	      			<div class="widget-header">
	      				<i class="icon-umbrella"></i><i class="icon-user"></i>
						...
						?>
						<span class="create_new"><?php echo anchor(base_url().ADMIN.'/subadmin/add?pagemode='.$pagestatus.'&modestatus='.$pagingstatus.'&sortingfied='.$fieldssort.'&sortype='.$ordersort,'Create',array('class'=>'btn btn-suc')); ?></span>
	  				</div> <!-- /widget-header -->
					<div class="widget-content">
						<ul class="nav nav-tabs">
							<li <?php if($this->uri->segment(3)=="" || $this->uri->segment(3)=="index") { ?>class="active" <?php } ?>><?php echo anchor(base_url().ADMIN.'/subadmin','All'); ?></li>
							<li <?php if($this->uri->segment(3)=="active") { ?>class="active" <?php } ?>><?php echo anchor(base_url().ADMIN.'/subadmin/active','Active'); ?></li>	
							<li <?php if($this->uri->segment(3)=="inactive") { ?>class="active" <?php } ?>><?php echo anchor(base_url().ADMIN.'/subadmin/inactive','Inactive'); ?></li>
						</ul>
						<?php 
						...
						/******** Pagination count ***********/ 
						 ...
						 if($order_seg == "asc") $order = "desc"; else $order = "asc";
							...
							$sort_def_img = img(base_url().'assets/img/admin/sort_both.png');
							 echo form_open(ADMIN.'/subadmin/'.$type.'/1');
							?>
							...
							<div class="quick-search-containers">
								<div class="input-group input-group-sm">
									...
									echo ' '.anchor(ADMIN.'/subadmin/'.$type.'/1/reset', 'Clear' ,array('title'=>'Clear', 'class'=>'btn btn-sm btn-default'));
									?>
									</span>
						...
						<!-- /widget -->
						<!-- /widget -->
					<?php echo form_open(base_url().ADMIN.'/subadmin/bulkautions/'.$pagestatus.'/'.$pagingstatus.'?sortingfied='.$fieldssort.'&sortype='.$ordersort); ?>
						  ...
						  <table class="table table-striped table-bordered" id="rowclick1">
							<thead>
							  <tr>
								...
								<th><?php $sort_url = base_url().ADMIN.'/subadmin/'.$type.'/'.$page_num.'/name/'.$order;   if($sort_seg == 'name') {  echo anchor($sort_url ,'Name'.$sort_img, $sort_att );  } else { echo anchor($sort_url,'Name'.$sort_def_img, $sort_att ); }?></th>	
								<th> <?php $sort_url = base_url().ADMIN.'/subadmin/'.$type.'/'.$page_num.'/email/'.$order;   if($sort_seg == 'email') {  echo anchor($sort_url ,'Email'.$sort_img, $sort_att );  } else { echo anchor($sort_url,'Email'.$sort_def_img, $sort_att ); }?></th>
								<th class="create-wt">  <?php $sort_url = base_url().ADMIN.'/subadmin/'.$type.'/'.$page_num.'/created/'.$order;   if($sort_seg == 'created') {  echo anchor($sort_url ,'Created'.$sort_img, $sort_att );  } else { echo anchor($sort_url,'Created'.$sort_def_img, $sort_att ); }?> </th>
							...
							?>
							  <tr class="check-select">
								...
								<?php
								if($vals['is_active']==1) {
								echo anchor(base_url().ADMIN.'/subadmin/disable/'.$vals['id'].'/'.$pagestatus.'/'.$pagingstatus.'?sortingfied='.$fieldssort.'&sortype='.$ordersort,'<i class="btn-icon-only icon-ok"> </i>',array('class'=>'btn btn-small btn-success','title'=>'Active'));
								} else { 
								echo anchor(base_url().ADMIN.'/subadmin/enable/'.$vals['id'].'/'.$pagestatus.'/'.$pagingstatus.'?sortingfied='.$fieldssort.'&sortype='.$ordersort,'<i class="btn-icon-only icon-remove"> </i>',array('class'=>'btn btn-danger btn-small','title'=>'Inactive'));
								?>
								<?php } ?>
								</td>
								<td class="td-actions">
								<?php 
								echo anchor(base_url().ADMIN.'/subadmin/view/'.$vals['id'].'?pagemode='.$pagestatus.'&modestatus='.$pagingstatus.'&sortingfied='.$fieldssort.'&sortype='.$ordersort,'<i class="btn-icon-only icon-list-alt"> </i>',array('class'=>'btn btn-big','title'=>'View'));
								echo anchor(base_url().ADMIN.'/subadmin/edit/'.$vals['id'].'?pagemode='.$pagestatus.'&modestatus='.$pagingstatus.'&sortingfied='.$fieldssort.'&sortype='.$ordersort,'<i class="btn-icon-only icon-edit"> </i>',array('class'=>'btn btn-big','title'=>'Edit'));
								echo anchor(base_url().ADMIN.'/subadmin/change_password/'.$vals['id'].'?pagemode='.$pagestatus.'&modestatus='.$pagingstatus.'&sortingfied='.$fieldssort.'&sortype='.$ordersort,'<i class="btn-icon-only icon-key"> </i>',array('class'=>'btn btn-big','title'=>'Change Password'));
								?>
								</td>
							  </tr>
							<?php }
