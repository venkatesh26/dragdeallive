            ...
            <div class="td-pb-row">
                <div class="td-pb-span12">
                    <div class="td-post-header">
                        <?php /************ Bread Crumb *****************/ echo $this->load->view('bread_crumb'); ?>
                    ...
                    <div class="td-ss-main-content">
                        ...
                        <div class="td-post-sharing td-post-sharing-top">
							...
							<?php $share_link="http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";?>
                                ...
                                <a onclick="window.open(this.href, 'mywin','left=50,top=50,width=600,height=350,toolbar=0'); return false;" href="http://www.linkedin.com/shareArticle?mini=true&url=<?php echo $share_link;?>&title=<?php echo ucwords($result['name']);?>&summary=&source=<?php echo base_url();?>" class="td-social-sharing-buttons td-social-pinterest"><i class="fa fa-linkedin"></i></a>
                            </div>
                        </div>
							<?php 
							if(!empty($result['image_name']) && file_exists('./'.$result['image_dir'].$result['image_name']))
							{
							   $img_src = thumb(FCPATH.$result['image_dir'].$result['image_name'],'500','255','blog_view_thumb');
							   $image = base_url().$result['image_dir'].'blog_view_thumb/'.$img_src;?>
								<div class="td-post-content" style="text-align:center;">
								<div class=td-post-featured-image><a href="<?php echo $image;?>" data-caption=""><img width=500 height=507 class="entry-thumb td-modal-image" src="<?php echo $image;?>" alt="" title=281 /></a></div>
								</div>
							<?php 
							}
						...
						<br/>Source: <a target="_blank" href="<?php echo $result['source_url'];?>"><?php echo $result['source_url'];?></a>
						<?php endif;?>
						<br/>
						<?php if($result['api_provider']!=''):?>
						<br/>Powered by: <a target="_blank" href="<?php echo $result['api_provider'];?>"><?php echo $result['api_provider'];?></a>
						<?php endif;?>
						<br/>
                        ...
                        <?php
							echo $this->load->view('news/view_share');
							echo $this->load->view('news/view_reviews');
						?>
					</footer>
					   <?php 
						echo $this->load->view('news/view_add_news_comments');
					   ?>
                    </div>
                </div>
                <?php echo $this->load->view('categories/related_news_category');?>
            </div>
        </article>
    </div>
</div>