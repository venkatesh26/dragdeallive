...
class Blogs extends CI_Controller {
	...
	public function index() {
		...
		if($this->uri->segment('1')=='blog-category-search' && $this->uri->segment('2')!='')
		{
		...
		$config['first_url'] = base_url().'blog/index'.$query_string;
		$config['per_page'] = '5';
		$config['num_links'] = '3';
		...
		#Get Blogs List
		$data["list"] = $this->blog_model->get_blog_list('all',$config['per_page'], $page,$category_name,1);
		$config['total_rows'] = $data["list"]['all_total_rows'];
		$this->pagination->initialize($config);
		$data["pagination_link"]= $this->pagination->create_links();
		
		$data['category_list']=$this->blog_model->get_blog_list_category('1');
		#BreadCrumb Push 
		$this->load->library('breadcrumbs');
		$this->breadcrumbs->push($this->site_name,base_url());
		$this->breadcrumbs->push("Blogs",base_url().'/blogs');	
		$data['total_count']=$config['total_rows'];
		$data['search_header_title']="Blogs";
		$data['main_content']=$this->load->view('blogs/list', $data,true);
	...
	public function view($id) {
		...
		$this->breadcrumbs->push($this->site_name,base_url());
		$this->breadcrumbs->push('Blogs', base_url().'blogs');
	    $this->breadcrumbs->push(ucfirst($r_category_name),base_url().'blog-category-search/'.url_title(strtolower($r_category_name)));	
		$this->breadcrumbs->push($getValues['name'], base_url().'blogs_view/view');
		$data['result'] = $getValues;	
		$data['main_content'] = 'blogs/view';
		...
		$data['main_content']=$this->load->view('blogs/view', $data,true);
		$this->load->view('layouts/default',$data);
	}
	...
	public function admin_index($type = null, $page_num =1,$sortfield='id',$order='desc') 
	{
		...
		$data['blogs'] = $this->blog_model->get_blogs( 1 , $conditions, $sort_f, $order, $limit_start ,$limit_end,1);        
		$config['total_rows'] =$this->blog_model->get_blogs( 0 , $conditions, '', '', '', '',1); 
	...
	public function add() 
	{
		...
		else 
		{
			...
			$this->data['categories']=$this->blog_model->get_blog_category('1');
