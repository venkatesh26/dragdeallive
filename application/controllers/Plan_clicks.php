<?php
class Plan_clicks extends CI_Controller {

    public function __construct()
    {
        parent::__construct();
        $this->load->model('plan_clicks_model');
    ...
    public function index($type = null, $page_num =1,$sortfield='id',$order='desc') {
		...
		$this->sorting = array('name' =>'plans.name','created' =>'plan_clicks.created','customer_name' =>'user_profiles.first_name');
		$config['base_url'] = base_url().ADMIN.'/plan_clicks/index';
		$data['indextitle']  = 'Plan Click List';
		$data['type'] = 'index';
		...
		$data['pages'] = $this->plan_clicks_model->get_plan_clicks( 1 , $conditions, $sort_f, $order, $limit_start ,$limit_end);   
		$config['total_rows'] =$this->plan_clicks_model->get_plan_clicks( 0 , $conditions, '', '', '', '');   

		if($config['total_rows'] <= $limit_end )
			if($page_num && $page_num != 1) 
				redirect(ADMIN.'/plan_clicks/'.$data['type'].'/'. ($page_num -1).'/'.$sortfield.'/'.$order );
		
		$this->pagination->initialize($config);  
		$data['per_page']  = $limit_start;
		
		$data['main_content'] = 'admin/plan_clicks/index';
