...
class Blog_categories extends CI_Controller {
    ...
    public function index($type = null, $page_num =1,$sortfield='id',$order='desc') { 
		...
		$this->sorting = array('name' =>'blog_category.name','created' =>'blog_category.created');
		if($type == 'active')
		{
			$conditions[] = array( 'direct'=>0, 'rule' => 'where', 'field'=>'blog_category.is_active', 'value' => 1 );	
			$config['base_url'] = base_url().ADMIN.'/blog_category/active';
			$data['indextitle']  = 'Blog Categories - Active List';
			$data['type'] = 'active';
		} else if($type == 'inactive') {
			$conditions[] = array( 'direct'=>0, 'rule' => 'where', 'field'=>'blog_category.is_active', 'value' => 0 );	
			$config['base_url'] = base_url().ADMIN.'/blog_category/inactive';
			$data['indextitle']  = 'Blog Categories - Inactive List';
			$data['type'] = 'inactive';
		}
		else
		{
			$conditions[] = array( 'direct'=>0, 'rule' => 'where', 'field'=>'blog_category.is_active !=', 'value' => 2 );	
			$config['base_url'] = base_url().ADMIN.'/blog_category/index';
			$data['indextitle']  = 'Blog Categories List';
			$data['type'] = 'index';
		}
		/*****pagination ********/ 
		...
		if($data['keyword']  )
		{
			$value = '( blog_category.name Like "%'.$data['keyword'].'%" )'; 
		...
		$data['blog_category'] = $this->blog_category_model->get_categories( 1 , $conditions, $sort_f, $order, $limit_start ,$limit_end);  
		$config['total_rows'] =$this->blog_category_model->get_categories( 0 , $conditions, '', '', '', '');   
		if($config['total_rows'] <= $limit_end )
			if($page_num && $page_num != 1) 
				redirect(ADMIN.'/blog_category/'.$data['type'].'/'. ($page_num -1).'/'.$sortfield.'/'.$order );
		$this->pagination->initialize($config);  
		$data['per_page']  = $limit_start;
				
		$data['main_content'] = 'admin/blog_category/index';
    ...
    public function add() {
		...
		$this->breadcrumbs->push('Blog Categories', base_url().ADMIN.'/blog_category');
		$this->breadcrumbs->push('Add', base_url().ADMIN.'/blog_category/add');
		$this->form_validation->set_rules('name', 'Name','trim|required|is_unique[blog_category.name]');
		if ($this->form_validation->run() == true) 
		{
			$success = $this->blog_category_model->add_new();
			if($success==true) {
		...
		$this->data['main_content'] = 'admin/blog_category/add';
	...
	{
		$getValues = $this->blog_category_model->get_values($id);
		...
		if(count($getValues)) {
			$this->load->helper('thumb_helper');
			$this->breadcrumbs->push('Blog Categories', base_url().ADMIN.'/blog_category');
			$this->breadcrumbs->push('Edit', base_url().ADMIN.'/blog_category/edit');
			
			if (isset($_POST) && !empty($_POST))
			{
				if(trim($this->input->post('name')) != $getValues['name']) {
					$is_unique =  '|is_unique[blog_category.name]' ;
				} else {
					$is_unique =  '' ;
				}
				...
				if ($this->form_validation->run() === true)
				{	
					$this->blog_category_model->edit($id);
					$this->session->set_flashdata('flash_message', $this->lang->line('record_update'));
					redirect(base_url().ADMIN.'/blog_category/'.$pageredirect.'/'.$pageno.'/'.$fieldsorts.'/'.$typesorts);
				}			
			}
			
			$this->data['message'] = (validation_errors() ? validation_errors() : $this->session->flashdata('message'));
			$this->data['blog_category'] = $getValues;
			$this->data['is_active']=$getValues['is_active'] ? 1 : 0 ; 
			$this->data['main_content'] = 'admin/blog_category/edit';
	...
	function update_status($id, $status, $pageredirect=null,$pageno){ 
		...
		$this->blog_category_model->update_status($id, $data);
		redirect(base_url().ADMIN.'/blog_category/'.$pageredirect.'/'.$pageno.'/'.$fieldsorts.'/'.$typesorts);
	}
