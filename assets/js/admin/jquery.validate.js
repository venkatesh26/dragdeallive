 ...
 *
 * $Id: jquery.validate.js,v 1.2 2011-07-07 09:51:11 palanisamy Exp $
 *
...
$.extend($.validator, { 
	...
	defaults: {
		...
		highlight: function( element, errorClass, validClass ) { 
			...
			$('div.select label').next('select.error').parents('div.select').addClass('error');
			$('div.select label').next('div').parents('div.select').addClass('error');    
            $('.custom-upload input').next('label.error').parents('.custom-upload').addClass('error');
		},
		unhighlight: function( element, errorClass, validClass ) {
			$(element).removeClass(errorClass).addClass(validClass);
			//$('div.select label').next('select.valid').parents('div.select').removeClass('error');
			$('select.valid').parents('div.select').removeClass('error');
			 $('.custom-upload input').next('label.error').parents('.custom-upload').removeClass('error');
		}
	},
	...
	prototype: {
		...
		idOrName: function(element) {
			return this.groups[element.id] || (this.checkable(element) ? element.name : element.id || element.id);
		},
				...
				$.ajax($.extend(true, {
					...
					success: function(response) {
						...
						if ( valid ) {
							...
							errors[element.id] = $.isFunction(message) ? message(value) : message;
