...
class Users_model extends CI_Model {
	...
	public function check_verify_code($vid) {
		...
		if($numRows == 1) {
			$update_vid = array('uid'=>'', 'is_email_confirmed'=>'1');
    ...
    function validate_users($user_name, $password ,$user_type=null) {
		$this->db->select('id,first_name,last_name,email,user_type,is_email_confirmed,is_active');
	...
	function check_user_available($email,$uid) {
		$this->db->where('email', $email);
		$this->db->where_in('user_type', array('1','4','2','3'));
		$query = $this->db->get('users');
	...
	public function getUsername($email=null) {
	    $this->db->select('users.id,users.email,users.first_name');
		$this->db->from('users');
		$this->db->where('users.email',$email);
		$query = $this->db->get();
		$users = $query->row_array(); 
