	...
	function show()
	{
		if ($this->breadcrumbs) {
			...
			foreach ($this->breadcrumbs as $key => $crumb) {
				...
				$last=array_keys($this->breadcrumbs);
				if (end($last) == $key) {
					$output .= $this->crumb_last_open . '' . $crumb['page'] . '' . $this->crumb_close;
				} else {
					$output .= $this->crumb_open.'<a href="' . $crumb['href'] . '" itemprop="url"><span itemprop="title">' . $crumb['page'] . '</span></a>'.$this->crumb_close.$this->crumb_divider;
				}
			}
